...
class Solution 
{
    public:
        int networkDelayTime(vector<vector<int>>& times, int n, int k) 
        {
            ...
            if(adj[k].size() == 0)
            {                     // If there is no outgoing edge from k, return -1.
                return -1;
            }
            else
            ...
            if(answer == INT_MAX)
            {      // Means we have missed some node.
                return -1;
            }
        return answer;              
    }
...
int main() {
    ...
    // Get user inputs for times, n, and k
    int numEdges;
    cout << "Enter the number of edges: ";
    cin >> numEdges;

    vector<vector<int>> times(numEdges);
    cout << "Enter the edges in the format (source, destination, weight):\n";
    for (int i = 0; i < numEdges; ++i) {
        int source, destination, weight;
        cin >> source >> destination >> weight;
        times[i] = {source, destination, weight};
    }
    ...
    // Call the networkDelayTime function
    int minTime = s.networkDelayTime(times, n, k);
